...
export const amazonNeglectedSql = ({
  ...
  if (details.searchTerm) {
    sql += `AND (p.sku ILIKE '%${details.searchTerm}%'
            OR apd.neglected_status ILIKE '%${details.searchTerm}%')
          `;
  }

  sql += `ORDER BY ${
    details.sort.columnName !==
    ('apd.neglected_status' as Dius.AmazonNeglectedProductsSortableColumns)
      ? `CASE WHEN apd.neglected_status = 'OPTIMIZE' THEN 1
          WHEN apd.neglected_status = 'DELETE_SKU' THEN 2
          WHEN apd.neglected_status = 'KEEP_SELLING' THEN 3
          ELSE 4
        END,`
      : ''
  }
        ${details.sort.columnName} ${details.sort.sortBy} NULLS LAST, p.id
         `;
